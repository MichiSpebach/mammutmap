...
test('default snapshotOptions', async () => {
    ...
    e2eUtil.expectImageToMatchSnapshot({image: imageIdentical, snapshotDirname: __dirname, snapshotIdentifier: 'util-test-ts-default-snapshot-options'})

    const imageBlurred = await fsPromises.readFile('./testE2e/util/util-test-ts-default-snapshot-options-snap-match-blurred.png')
    e2eUtil.expectImageToMatchSnapshot({image: imageBlurred, snapshotDirname: __dirname, snapshotIdentifier: 'util-test-ts-default-snapshot-options'})
return // TODO: below produces "2 snapshots failed" that is confusing, find way to catch this and reactivate
    ...
    e2eUtil.expectImageNotToMatchSnapshot({image: imageFarMoved, snapshotDirname: __dirname, snapshotIdentifier: 'util-test-ts-default-snapshot-options'})
    ...
    e2eUtil.expectImageNotToMatchSnapshot({image: imageSlightlyMoved, snapshotDirname: __dirname, snapshotIdentifier: 'util-test-ts-default-snapshot-options'})
})

...
test('getNearestGridPositionOfOtherTransform', async () => {
  ...
  const result: LocalPosition = await setupScenario().transform.getNearestGridPositionOfOtherTransform(new ClientPosition(749, 447), scenario.otherTransform)
  expect(result.percentX).toBeCloseTo(62.5, 10)
  expect(result.percentY).toBeCloseTo(73, 10)
})

test('getNearestGridPositionOf rounds to multiple of 2', () => {
  const result: LocalPosition = setupScenario().transform.roundToGridPosition(new LocalPosition(3, 78.99))
  expect(result.percentX).toBe(4)
  expect(result.percentY).toBe(78)
})
